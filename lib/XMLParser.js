...
XMLParser.prototype = {
	...
	processData : function(xmlString, currentNode, currentPos) {
		...
		while (-1 < currentPos && currentPos < xmlString.length) {
			...
			if (xmlString.substr(tagStart, tagEnd-tagStart+1).indexOf("<?", 0) != -1 || 
				xmlString.substr(tagStart, tagEnd-tagStart+1).indexOf("<!", 0) != -1) {
				// Unhandled tag
				
			} else if (xmlString.substr(tagStart, tagEnd+1-tagStart).indexOf("/>") != -1) {
